...
export function verifySchema() {
  ...
  const requiredTables = ['Album', 'Photo', 'AlbumPhoto'];
  const result = db.exec(`
    SELECT name FROM sqlite_master
    WHERE type='table' AND name IN ('${requiredTables.join("','")}')
    ORDER BY name
  `);

  if (result.length === 0 || result[0].values.length !== requiredTables.length) {
    throw new Error('Schema verification failed: Not all required tables exist');
  }
