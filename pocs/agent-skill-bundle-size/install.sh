...
SRC="$(cd "$(dirname "$0")" && pwd)/skills/bundle-size"
...
chmod +x "$CLAUDE_DEST/scripts/bundlesize.mjs"
echo "Installed bundle-size to Claude Code: $CLAUDE_DEST"

if [ -d "$HOME/.codex" ]; then
  rm -rf "$CODEX_DEST"
  mkdir -p "$(dirname "$CODEX_DEST")"
  cp -R "$CLAUDE_DEST" "$CODEX_DEST"
  echo "Installed bundle-size to Codex: $CODEX_DEST"
else
  echo "Codex not found at $HOME/.codex - skipping Codex install"
fi

echo "Done. Run /bundle-size in a frontend project."
