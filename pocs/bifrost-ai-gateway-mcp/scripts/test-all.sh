...
( cd "$ROOT/mcp" && "$PYTHON" -m unittest discover -s tests ) || fail "mcp tests failed"
...
status=0
BIFROST_PORT="$(service_port bifrost)" APP_PORT="$(service_port app)" MCP_PORT="$(service_port mcp)" "$PYTHON" -m unittest -v "$ROOT/tests/integration_test.py" || status=$?
