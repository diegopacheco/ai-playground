...
- Create `run.sh` that starts the database, backend, and frontend. The script MUST log the default user/pass credentials for the application to stdout on startup.
- Run `run.sh` and verify that the database, backend, and frontend all start successfully and are reachable. If any component fails to start, fix the issue, record it in `mistakes.md`, and re-run `run.sh`.
- If ANY build step fails: fix the issue, record it in `mistakes.md`, and re-verify.
- Do NOT proceed to Phase 2 until the full build is green, `build.sh` produces zero errors and zero warnings, and `run.sh` starts all components successfully.
