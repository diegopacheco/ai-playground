...
def process_runner(command: List[str], cwd: Optional[str] = None) -> str:
    result = subprocess.run(command, stdin=subprocess.DEVNULL, capture_output=True, text=True, check=False, env=command_environment(command[0]), cwd=cwd)
