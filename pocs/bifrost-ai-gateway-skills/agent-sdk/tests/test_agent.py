...
class AgentTest(unittest.TestCase):
    def test_providers_build_argument_arrays_without_a_shell(self):
        ...
        self.assertEqual(["ollama", "run", "--nowordwrap", "--flag", "model-a", "hello world"], commands[3])
...
class ProcessRunnerTest(unittest.TestCase):
    def test_cli_runs_inside_the_given_directory_so_it_discovers_the_skills_installed_there(self):
        with TemporaryDirectory() as directory:
            self.assertEqual(Path(directory).resolve(), Path(process_runner(["pwd"], cwd=directory).strip()).resolve())
