...
type Sort = "alpha" | "alphaDesc" | "newest" | "oldest" | "progress"
...
export function LibraryPage() {
  ...
  const media = useMemo(() => {
    const items = (library.data?.media || []).filter(item => (filter === "all" || item.type === filter) && item.title.toLowerCase().includes(query.toLowerCase()))
  ...
  return <div className="page library-page">
    <section className="page-heading"><div><span className="eyebrow">Collected & remembered</span><h1>My library</h1><p>Every title you are watching, finished or saving for later.</p></div><div className="heading-actions"><button className="button secondary import-button" onClick={() => actions.syncMetadata.mutate()} disabled={actions.syncMetadata.isPending}><Icon name="film" size={17}/>{actions.syncMetadata.isPending ? "Loading artwork…" : "Refresh artwork"}</button><button className="button secondary import-button" onClick={() => setImportOpen(true)}><Icon name="upload" size={17}/>Import TV Time</button></div></section>
    <section className="library-toolbar"><div className="segmented">{(["all", "movie", "show"] as Filter[]).map(value => <button key={value} className={filter === value ? "active" : ""} onClick={() => { setFilter(value); setPage(0) }}>{value === "all" ? "Everything" : value === "movie" ? "Movies" : "TV shows"}</button>)}</div><label className="library-search"><Icon name="search" size={16}/><input value={query} onChange={event => { setQuery(event.target.value); setPage(0) }} placeholder="Filter your library"/></label><label className="sort-select"><span>Sort</span><select value={sort} onChange={event => { setSort(event.target.value as Sort); setPage(0) }}><option value="alpha">Alphabetical</option><option value="alphaDesc">Reverse alphabetical</option><option value="newest">Recently added</option><option value="oldest">Oldest added</option><option value="progress">Highest progress</option></select></label><span>{media.length} titles</span></section>
