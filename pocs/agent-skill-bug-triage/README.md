...
An agent skill that takes a single bug — from the branch you are on, a Jira or Linear url, a bug id, or a sentence you type — reads the real codebase to find its cause, and writes a self-contained light-theme HTML report to a temp folder.
...
1. You run `/bug-triage` with a branch, a tracker url, a bug id, or a description.
2. The skill reads the target — `git diff`, `git log`, or WebFetch on the issue url.
...
| Feature | Why |
|---|---|
| Four ways in — branch, url, bug id, description | You triage a bug from wherever it reached you, not from one tool |
...
| bash for install / uninstall | Copying a folder is a copy, not a build step |
...
- **The skill never edits the project.** It writes the report folder and nothing else — triage is a read of the code, not a change to it.
...
It runs the sample's existing suite (green while the bug is live), runs the reproduction test (must fail), renders the report, and checks all nine sections and the tracker link are in the HTML.
...
├── test.sh                      end to end: suite, reproduction, render, checks
└── README.md
```
