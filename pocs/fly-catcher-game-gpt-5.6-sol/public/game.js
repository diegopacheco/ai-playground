...
function startRound() {
  if (Date.now() - state.lastPacketAt >= 2200) return
  state.mode = 'running'
  state.score = 0
  state.hits = 0
  state.chain = 1
...
function endRound(stopped) {
  ...
  cardAction.disabled = Date.now() - state.lastPacketAt >= 2200
...
window.addEventListener('keydown', (event) => {
  const step = 28
  if (event.key === 'ArrowLeft') state.targetAim.x = Math.max(35, state.targetAim.x - step)
  if (event.key === 'ArrowRight') state.targetAim.x = Math.min(width - 35, state.targetAim.x + step)
  if (event.key === 'ArrowUp') state.targetAim.y = Math.max(45, state.targetAim.y - step)
  if (event.key === 'ArrowDown') state.targetAim.y = Math.min(height - 80, state.targetAim.y + step)
  if (event.code === 'Space') {
...
cardAction.addEventListener('click', startRound)
stopButton.addEventListener('click', () => endRound(true))
