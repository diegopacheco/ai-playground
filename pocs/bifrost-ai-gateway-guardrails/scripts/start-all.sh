...
[ -f "$PLUGIN" ] || fail "guardrails plugin missing, run ./scripts/setup.sh first"
mkdir -p "$RUN/bifrost"
sed -e "s/__BRIDGE_PORT__/$(service_port bridge)/g" -e "s|__PLUGIN_PATH__|$PLUGIN|g" "$ROOT/bifrost/config.template.json" >"$RUN/bifrost/config.json"
